...
namespace engine {
	...
	namespace graphics {
		utils::StrongHandle<Framebuffer> Framebuffer::Create(Vec2i size, InternalTextureFormat format, FBOFlags flags) {
			return dynamic_cast<Framebuffer *>(new engine::external::opengl::OpenGLFramebuffer(size, format, flags));
		}
	}

	namespace external { namespace opengl {

		OpenGLFramebuffer::OpenGLFramebuffer(Vec2i size, graphics::InternalTextureFormat format, graphics::FBOFlags flags) :
			...
			glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, numSamples, OpenGLHelper::GetOpenGLType(format), size.x, size.y, false);
