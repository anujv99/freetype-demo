...
namespace engine {
	...
	namespace external { namespace glfw {
		...
		GLFWWindow::GLFWWindow() : m_IsCreated(false), m_Data{ 0u, 0u, "", nullptr }, m_Window(nullptr) {
			m_Data.EventFunc = [](engine::core::events::Event &) -> void {};	//Assigning to reduce checking for every event
		}
		...
		void GLFWWindow::SetEventFunction(std::function<void(engine::core::events::Event &)> eventFunc) {
			if (eventFunc) {
		...
		void GLFWWindow::SetCallbacks() {
			glfwSetWindowCloseCallback(m_Window, [](::GLFWwindow * window) -> void {
				...
				engine::core::events::WindowCloseEvent e;
				data->EventFunc(e);
			});

			glfwSetWindowSizeCallback(m_Window, [](::GLFWwindow * window, int width, int height) -> void {
				WindowData * data = (WindowData *)glfwGetWindowUserPointer(window);
				engine::core::events::WindowResizeEvent e(width, height);
				data->EventFunc(e);
			...
			glfwSetWindowPosCallback(m_Window, [](::GLFWwindow * window, int x, int y) -> void {
				WindowData * data = (WindowData *)glfwGetWindowUserPointer(window);
				engine::core::events::WindowMoveEvent e(x, y);
				data->EventFunc(e);
			});
			...
			glfwSetKeyCallback(m_Window, [](::GLFWwindow * window, int keyCode, int scanCode, int action, int mods) -> void {
				...
				case GLFW_PRESS:
				{
					engine::core::events::KeyPressedEvent e(keyCode, false);
					data->EventFunc(e);
				...
				case GLFW_REPEAT:
				{
					engine::core::events::KeyPressedEvent e(keyCode, true);
					data->EventFunc(e);
					break;
				}
				case GLFW_RELEASE:
				{
					engine::core::events::KeyReleasedEvent e(keyCode);
					data->EventFunc(e);
					break;
				}
				}
			...
			glfwSetMouseButtonCallback(m_Window, [](::GLFWwindow * window, int button, int action, int mods) -> void {
				...
				case GLFW_PRESS:
				{
					engine::core::events::MouseButtonPressedEvent e(button);
					data->EventFunc(e);
					break;
				}
				case GLFW_RELEASE:
				{
					engine::core::events::MouseButtonReleasedEvent e(button);
					data->EventFunc(e);
					break;
				}
				}
			...
			glfwSetCursorPosCallback(m_Window, [](::GLFWwindow * window, double x, double y) -> void {
				WindowData * data = (WindowData *)glfwGetWindowUserPointer(window);
				engine::core::events::MouseMovedEvent e((float)x, (float)y);
				data->EventFunc(e);
			});

			glfwSetScrollCallback(m_Window, [](::GLFWwindow * window, double xOffset, double yOffset) -> void {
				WindowData * data = (WindowData *)glfwGetWindowUserPointer(window);
				engine::core::events::MouseScrolledEvent e((float)xOffset, (float)yOffset);
				data->EventFunc(e);
			});

			glfwSetCharCallback(m_Window, [](::GLFWwindow * window, unsigned int character) -> void {
				WindowData * data = (WindowData *)glfwGetWindowUserPointer(window);  
				engine::core::events::CharacterEvent e((char)character);
				data->EventFunc(e);
			});
		}
